// 增加：判断是否出现可视区時先判断容器是否在可视区，如果容器在可视区再判断图片是否再可视区
// 当容器大小大于浏览器可视区大小時，要比较图片位置和可视区位置的关系
// 用户只提供容器時，应该遍历所有的元素找到所有的img并处理
// -------------类要在调用之前引入---------------
// HzLazyLoad构造函数   selector：使用lazyload的img上的class   option:自定义配置{placeholder,container,distance,errorSrc,id}
let HzLazyLoad = window.HzLazyLoad = (function() {
    function HzLazyLoad(option) {
        ...
        this.selector = (option && option.className) || 'lazy-load'; // 使用lazyload的图片上的class
        this._container = (option && option.container && document.getElementById(option.container)) || document.body; // 容器
